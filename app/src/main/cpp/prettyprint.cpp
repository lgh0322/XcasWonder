...
namespace xcas {
    ...
    PrettyPrint::PrettyPrint(int windowsize, int fontsize, giac::gen gen) : Equation() {
        ...
        status = FT_New_Memory_Face(library, Helvetica_Regular_ttf, Helvetica_Regular_ttf_len, 0,
                                    &face_helvetica_regular);
    ...
    void PrettyPrint::draw(cairo_surface_t *cs, double r, double g, double b) {
        ...
        Equation_draw(data, rightx, lowery + height - HORRIBLE_FIX / 2, width,
                      -height - HORRIBLE_FIX, this);

        cairo_destroy(cairo);
        cairo_surface_destroy(cs);
    }

    void PrettyPrint::draw(cairo_surface_t *cs, unsigned char **png_array, int *lenght, double r,
                           double g, double b) {
    ...
    void
    PrettyPrint::check_fl_draw(const char *ch, int i0, int j0, int imin, int jmin, int di, int dj,
    ...
    void
    PrettyPrint::check_fl_rectf(int i0, int j0, int i1, int j1, int imin, int jmin, int di, int dj,
                                int delta_i, int delta_j) {
    ...
    void
    PrettyPrint::check_fl_line(int i0, int j0, int i1, int j1, int imin, int jmin, int di, int dj,
                               int delta_i, int delta_j) {
        cairo_move_to(cairo, i0, j0);
