...
#define SINGLE_FMT_BUFFER_SIZE 32
    ...
    while (*f != '\0') {
        ...
        switch (*f | length_modifier) {
            ...
            case 'X':
                if (var_width) {
                    ...
                    snprintf(buffer, sizeof buffer,
                             single_fmt, width, va_arg(ap,
                    int));
                } else {
                    snprintf(buffer, sizeof buffer, single_fmt, va_arg(ap,
                    int));
                }
                break;
            ...
            case 'f':
            case 'g':
                _trace_dtostr(buffer, sizeof buffer, va_arg(ap,
                double));
                break;
            case 'c':
                buffer[0] = va_arg(ap,
...
void
cairo_rel_curve_to(cairo_t *cr, double dx1, double dy1, double dx2, double dy2, double dx3,
                   double dy3) {
    _enter_trace();
...
void
cairo_select_font_face(cairo_t *cr, const char *family, cairo_font_slant_t slant,
...
cairo_surface_t *
cairo_image_surface_create_for_data(unsigned char *data, cairo_format_t format, int width,
...
cairo_pattern_t *
cairo_pattern_create_radial(double cx0, double cy0, double radius0, double cx1, double cy1,
                            double radius1) {
    cairo_pattern_t *ret;
    long pattern_id;
...
void
cairo_pattern_add_color_stop_rgb(cairo_pattern_t *pattern, double offset, double red, double green,
                                 double blue) {
    _enter_trace();
    _emit_line_info ();
...
void
cairo_pattern_add_color_stop_rgba(cairo_pattern_t *pattern, double offset, double red, double green,
