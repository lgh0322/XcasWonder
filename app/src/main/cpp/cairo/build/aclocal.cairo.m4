...
if cmp -s "$_tmp_file" "$_config_file";
then
        AC_MSG_NOTICE(
[
$_config_file is
...
the macro
.
AC_DEFUN([CAIRO_CC_TRY_LINK_WITH_ENV_SILENT],[
dnl
...
if test "x$cairo_cc_stderr" != "x";
then
        cairo_cc_flag = no
fi

if test "x$cairo_cc_flag" = "xyes";
then
        ifelse(
[$3], , :, [$3])
else
...
dnl check
compiler flags
possibly using -Werror if available.
...
[for
native atomic
primitives], cairo_cv_atomic_primitives,
[
...
if test "x$cairo_cv_atomic_primitives" = "xnone";
then
        AC_CHECK_HEADER(
[atomic_ops.h],
...
if test "x$cairo_cv_atomic_primitives" =
xIntel;
then
...
if test "x$cairo_cv_atomic_primitives" = "xlibatomic-ops";
then
...
ops require
a memory
barrier], cairo_cv_atomic_op_needs_memory_barrier,
[
case
$host_cpu in
i?86)
...
if test "x$cairo_cv_atomic_op_needs_memory_barrier" = "xyes";
then
