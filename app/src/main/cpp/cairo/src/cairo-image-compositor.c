...
composite_boxes(void *_dst,
    ...
    pixman_image_t *mask = abstract_mask ? ((cairo_image_source_t *) abstract_mask)->pixman_image
                                         : NULL;
...
_cairo_image_finish_spans_and_zero(void *abstract_renderer) {
    ...
    if (r->u.mask.extents.y < r->u.mask.extents.height)
        memset(r->u.mask.data, 0,
               (r->u.mask.extents.height - r->u.mask.extents.y) * r->u.mask.stride);
    ...
    if (likely(h == 1)) {
        ...
        do {
            if (spans[0].coverage) {
                ...
                do {
                    int len = spans[1].x - spans[0].x;
                    uint16_t *d = (uint16_t *) (r->u.fill.data + r->u.fill.stride * yy +
                                                spans[0].x * 2);
                    while (len--)
                        *d++ = r->u.fill.pixel;
                    yy++;
    ...
    if (likely(h == 1)) {
        do {
            if (spans[0].coverage) {
                int len = spans[1].x - spans[0].x;
                if (len > 32) {
                    pixman_fill((uint32_t *) r->u.fill.data, r->u.fill.stride / sizeof(uint32_t),
                                r->bpp,
                ...
                if (a == 0xff) {
                    if (len > 31) {
                        pixman_fill((uint32_t *) r->u.fill.data,
                                    r->u.fill.stride / sizeof(uint32_t), r->bpp,
                                    spans[0].x, y, len, 1, r->u.fill.pixel);
                    } else {
                        uint32_t *d = (uint32_t *) (r->u.fill.data + r->u.fill.stride * y +
                                                    spans[0].x * 4);
    ...
    if (likely(h == 1)) {
        ...
        do {
            ...
            if (a) {
                int yy = y, hh = h;
                do {
                    uint32_t *s = (uint32_t *) (r->u.blit.src_data + yy * r->u.blit.src_stride +
                                                spans[0].x * 4);
                    uint32_t *d = (uint32_t *) (r->u.blit.data + yy * r->u.blit.stride +
                                                spans[0].x * 4);
                    int len = spans[1].x - spans[0].x;
                    if (a == 0xff) {
                        if (len == 1)
