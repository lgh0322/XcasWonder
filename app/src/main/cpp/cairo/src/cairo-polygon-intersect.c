        ...
        if (_cairo_int64_ge (qr.rem, den_det)) {
            qr.quo = _cairo_int64_add (qr.quo,
                                       _cairo_int32_to_int64(
                                               _cairo_int64_negative(qr.quo) ? -1 : 1));
        } else
    ...
    while ((event = _cairo_bo_event_dequeue(&event_queue))) {
        ...
        switch (event->type) {
            case CAIRO_BO_EVENT_TYPE_START:
                ...
                if (left != NULL) {
                    status = event_queue_insert_if_intersect_below_current_y(&event_queue, left,
                                                                             e1);
                    if (unlikely (status))
                        goto unwind;
                }
                ...
                if (right != NULL) {
                    status = event_queue_insert_if_intersect_below_current_y(&event_queue, e1,
                                                                             right);
                    if (unlikely (status))
                        goto unwind;
                }
