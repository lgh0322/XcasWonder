            ...
            if (font_subset->latin_to_subset_glyph_index[i] > 0) {
                if (font_subset->glyph_names != NULL) {
                    _cairo_output_stream_printf(surface->final_stream,
                                                "Encoding %d /%s put\n",
                                                i,
                                                font_subset->glyph_names[font_subset->latin_to_subset_glyph_index[i]]);
                } else {
                    _cairo_output_stream_printf(surface->final_stream,
                                                "Encoding %d /g%ld put\n", i,
                                                font_subset->latin_to_subset_glyph_index[i]);
                }
            }
        }
    } else {
...
_cairo_ps_surface_emit_type3_font_subset(cairo_ps_surface_t *surface,
    ...
    cairo_status_t status;
    unsigned int i;
    cairo_box_t font_bbox = {{0, 0},
                             {0, 0}};
    cairo_box_t bbox = {{0, 0},
                        {0, 0}};
    cairo_surface_t *type3_surface;
    double width;
...
mask_supported(cairo_ps_surface_t *surface,
    ...
    if (mask->type == CAIRO_PATTERN_TYPE_SURFACE) {
        ...
        if (surface_pattern->surface->type == CAIRO_SURFACE_TYPE_IMAGE) {
            ...
            if (_cairo_ps_surface_analyze_surface_pattern_transparency(surface, mask, extents) ==
...
_cairo_ps_surface_analyze_operation(cairo_ps_surface_t *surface,
     ...
     */
    if (pattern->type == CAIRO_PATTERN_TYPE_SURFACE ||
        pattern->type == CAIRO_PATTERN_TYPE_RASTER_SOURCE)
...
_cairo_ps_surface_emit_image(cairo_ps_surface_t *surface,
    ...
    if (image->format != CAIRO_FORMAT_RGB24 &&
        ...
        surf = _cairo_image_surface_create_with_content(
                cairo_surface_get_content(&image_surf->base),
                image_surf->width,
                image_surf->height);
        image = (cairo_image_surface_t *) surf;
        if (surf->status) {
    ...
    if (use_mask) {
        ...
        if (!stencil_mask) {
            _cairo_output_stream_printf(surface->stream,
                                        "%s setcolorspace\n",
                                        color == CAIRO_IMAGE_IS_COLOR ? "/DeviceRGB"
                                                                      : "/DeviceGray");
        }
        _cairo_output_stream_printf(surface->stream,
                                    "8 dict dup begin\n"
                                    ...
                                    interpolate,
                                    color == CAIRO_IMAGE_IS_MONOCHROME ? 1 : 8,
                                    stencil_mask ? "1 0" : color == CAIRO_IMAGE_IS_COLOR
                                                           ? "0 1 0 1 0 1" : "0 1");
        if (surface->use_string_datasource) {
            _cairo_output_stream_printf(surface->stream,
                                        "  /DataSource {\n"
...
_cairo_ps_surface_emit_surface(cairo_ps_surface_t *surface,
    ...
    if (source_surface->type == CAIRO_SURFACE_TYPE_RECORDING) {
        if (source_surface->backend->type == CAIRO_SURFACE_TYPE_SUBSURFACE) {
            ...
            status = _cairo_ps_surface_emit_recording_subsurface(surface, sub->target,
                                                                 &sub->extents);
        } else {
