    ...
    if (scaled_font->unscaled_hfont == NULL) {
        ...
        if (!SelectObject(hdc, scaled_hfont))
            return _cairo_win32_print_gdi_error(
                    "_win32_scaled_font_get_unscaled_hfont:SelectObject");

        otm_size = GetOutlineTextMetrics(hdc, 0, NULL);
        if (!otm_size)
            return _cairo_win32_print_gdi_error(
                    "_win32_scaled_font_get_unscaled_hfont:GetOutlineTextMetrics");
        ...
        if (!scaled_font->unscaled_hfont)
            return _cairo_win32_print_gdi_error(
                    "_win32_scaled_font_get_unscaled_hfont:CreateIndirect");
    }
...
_cairo_win32_scaled_font_select_unscaled_font(cairo_scaled_font_t *scaled_font,
    ...
    status = _win32_scaled_font_get_unscaled_hfont((cairo_win32_scaled_font_t *) scaled_font, hdc,
                                                   &hfont);
    if (status)
        return status;
...
_cairo_win32_scaled_font_type1_text_to_glyphs(cairo_win32_scaled_font_t *scaled_font,
    ...
    if (GetGlyphIndicesW(hdc, utf16, n16, glyph_indices, 0) == GDI_ERROR) {
        status = _cairo_win32_print_gdi_error(
                "_cairo_win32_scaled_font_type1_text_to_glyphs:GetGlyphIndicesW");
        goto FAIL3;
    }
...
_cairo_win32_scaled_font_set_metrics(cairo_win32_scaled_font_t *scaled_font) {
    ...
    if (scaled_font->preserve_axes ||
        scaled_font->base.options.hint_metrics == CAIRO_HINT_METRICS_OFF) {
        ...
        extents.height =
                (metrics.tmHeight + metrics.tmExternalLeading) / scaled_font->logical_scale;
        ...
        extents.height =
                (double) (metrics.tmHeight + metrics.tmExternalLeading) / scaled_font->em_square;
...
_cairo_win32_scaled_font_init_glyph_metrics(cairo_win32_scaled_font_t *scaled_font,
    ...
    if (scaled_font->is_bitmap) {
        ...
        if (!GetCharWidth32(hdc, charIndex, charIndex, &width)) {
            status = _cairo_win32_print_gdi_error(
                    "_cairo_win32_scaled_font_init_glyph_metrics:GetCharWidth32");
            width = 0;
        }
        cairo_win32_scaled_font_done_font(&scaled_font->base);
        if (status)
        ...
        extents.x_bearing = 0;
        extents.y_bearing =
                scaled_font->base.ctm.yy * (-font_extents.ascent / scaled_font->y_scale);
        extents.width = width / (WIN32_FONT_LOGICAL_SCALE * scaled_font->x_scale);
        extents.height = scaled_font->base.ctm.yy * (font_extents.ascent + font_extents.descent) /
                         scaled_font->y_scale;
        extents.x_advance = extents.width;
        extents.y_advance = 0;
    } else if (scaled_font->preserve_axes &&
               scaled_font->base.options.hint_metrics != CAIRO_HINT_METRICS_OFF) {
...
_cairo_win32_scaled_font_load_truetype_table(void *abstract_font,
    ...
    tag = (tag & 0x000000ff) << 24 | (tag & 0x0000ff00) << 8 | (tag & 0x00ff0000) >> 8 |
          (tag & 0xff000000) >> 24;
    status = cairo_win32_scaled_font_select_font(&scaled_font->base, hdc);
    if (status)
        return status;
...
cairo_win32_scaled_font_select_font(cairo_scaled_font_t *scaled_font,
    ...
    if (!old_mode) {
        status = _cairo_win32_print_gdi_error(
                "cairo_win32_scaled_font_select_font:SetGraphicsMode");
        SelectObject(hdc, old_hfont);
        return status;
    }
