...
_cairo_win32_display_surface_create_similar(void *abstract_src,
    ...
    if (!(src->is_dib || content & CAIRO_CONTENT_ALPHA)) {
        /* try to create a ddb */
        new_surf = cairo_win32_surface_create_with_ddb(src->win32.dc, CAIRO_FORMAT_RGB24, width,
                                                       height);
...
*/
    ...
    if (ddb_dc == NULL) {
        new_surf = (cairo_win32_display_surface_t *) _cairo_surface_create_in_error(
