...
static cairo_status_t
_cairo_path_to_qpainterpath_curve_to(void *closure, const cairo_point_t *p0,
...
_cairo_qt_surface_create_similar(void *abstract_surface,
    ...
    D(fprintf(stderr, "q[%p] create_similar: %d %d [%d] -> ", abstract_surface, width, height,
              content));
...
struct PatternToBrushConverter {
        ...
        if (pattern->type == CAIRO_PATTERN_TYPE_SOLID) {
            ...
            if (surface->type == CAIRO_SURFACE_TYPE_QT) {
                ...
                if (surface->type == CAIRO_SURFACE_TYPE_IMAGE) {
                    ...
                    if (_cairo_surface_acquire_source_image(surface, &isurf, &image_extra) ==
            ...
            if (pattern->type == CAIRO_PATTERN_TYPE_LINEAR) {
                ...
                if (r0 == r1) {
                     ...
                     */
                    if ((extend == CAIRO_EXTEND_REFLECT || extend == CAIRO_EXTEND_REPEAT) &&
                        r0 > 0.0) {
                        double r_org = r1;
                        double r, x, y;
    ...
    ~PatternToBrushConverter() {
        if (mAcquiredImageParent)
            _cairo_surface_release_source_image(mAcquiredImageParent, mAcquiredImage,
                                                mAcquiredImageExtra);
    }
