...
i965_shader_acquire_solid_surface(i965_shader_t *shader,
    ...
    if (image->format != CAIRO_FORMAT_ARGB32) {
        ...
        argb = *(uint32_t * )((cairo_image_surface_t * )
        pixel)->data;
        cairo_surface_destroy(pixel);
    } else {
                ...
                if (s != shader->target) {
                    ...
                    clone = (i965_surface_t *)
                            i965_surface_create_internal(
                                    (cairo_drm_device_t *) s->intel.drm.base.device,
...
i965_shader_copy_channel_constants(i965_shader_t *shader,
    ...
    if (channel->base.constants_size) {
        assert(shader->constants_size + channel->base.constants_size <
               ARRAY_LENGTH(shader->constants));
...
emit_surface_state(i965_device_t *device,
    ...
    i965_emit_relocation(device, &device->surface,
                         ...
                         offset + offsetof(
    struct brw_surface_state, ss1.base_addr));
    return offset;
}
...
i965_emit_composite(i965_device_t *device,
    ...
    if (i965_shader_needs_constants_update(shader, device)) {
        ...
        */
        ...
        assert(((sizeof(float) * shader->constants_size + 31) & -32) ==
               32 * i965_shader_const_urb_length(shader));
