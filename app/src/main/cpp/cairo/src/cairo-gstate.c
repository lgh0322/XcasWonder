...
_cairo_gstate_redirect_target(cairo_gstate_t *gstate, cairo_surface_t *child) {
    ...
    gstate->clip = _cairo_clip_copy_with_translation(gstate->next->clip,
                                                     child->device_transform.x0 -
                                                     gstate->parent_target->device_transform.x0,
...
_cairo_gstate_stroke(cairo_gstate_t *gstate, cairo_path_fixed_t *path) {
    ...
    if (_cairo_stroke_style_dash_can_approximate(&gstate->stroke_style, &gstate->ctm,
                                                 gstate->tolerance)) {
        style.dash = dash;
...
_cairo_gstate_show_text_glyphs(cairo_gstate_t *gstate,
    ...
    cairo_text_cluster_t stack_transformed_clusters[CAIRO_STACK_ARRAY_LENGTH (
