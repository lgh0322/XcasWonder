...
_cairo_pattern_notify_observers(cairo_pattern_t *pattern,
    ...
    cairo_list_foreach_entry (pos, cairo_pattern_observer_t, &pattern->observers, link) pos->notify(
                pos, pattern, flags);
}
...
_cairo_pattern_hash(const cairo_pattern_t *pattern) {
    ...
    switch (pattern->type) {
        ...
        case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
            return _cairo_raster_source_pattern_hash(hash,
...
_cairo_debug_print_raster_source_pattern(FILE *file,
                                         const cairo_raster_source_pattern_t *raster) {
    printf("  content: %x, size %dx%d\n", raster->content, raster->extents.width,
...
_cairo_debug_print_pattern(FILE *file, const cairo_pattern_t *pattern) {
    ...
    switch (pattern->type) {
        ...
        case CAIRO_PATTERN_TYPE_SOLID:
            break;
        case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
            _cairo_debug_print_raster_source_pattern(file,
                                                     (cairo_raster_source_pattern_t *) pattern);
            break;
        case CAIRO_PATTERN_TYPE_SURFACE:
