...
wm_name.
value = (unsigned char *) argv[0];
wm_name.
encoding = XA_STRING;
wm_name.
format = 8;
wm_name.
nitems = strlen(argv[0]) + 1;
icon_name = wm_name;
gc_mask = 0;
while (*++argv) {
if (!
...
input = True;
classHints.
res_name = init_argv[0];
classHints.
res_class = init_argv[0];
...
XSetWMProperties (dpy, win,
        ...
        &sizeHints, &wmHints,
0);
XSetWMProtocols (dpy, win, &wm_delete_window,
1);
XMapWindow (dpy, win
);
for (;;) {
...
.type != Expose) {
HasExpose = 0;
handle_expose (dpy, &eev
.xexpose);
}
switch (ev.type) {
case ConfigureNotify:
handle_configure (dpy, &ev
.xconfigure);
break;
case Expose:
if (
...
*) &ev, quit_string, sizeof (quit_string), 0, 0) == 1) {
switch (quit_string[0]) {
case 'q':
exit (0);
case 'c':
XClearArea (dpy, ev
.xkey.window, 0, 0, 0, 0, True);
break;
}
}
break;
