...
find_ranges(struct chart *chart) {
    ...
    printf("%d: slow[%d] average: %f, fast[%d] average: %f, %f\n",
           num_values, slow_count, slow_sum / slow_count, fast_count, fast_sum / fast_count,
           sum / num_values);
}
...
add_absolute_lines(struct chart *c) {
    ...
    do {
        ...
        cairo_move_to(c->cr, 1 - extents.x_bearing,
                      floor(y) - (extents.height / 2 + extents.y_bearing) + .5);
        cairo_show_text(c->cr, buf);

        cairo_move_to(c->cr, c->width - extents.width - 1,
                      floor(y) - (extents.height / 2 + extents.y_bearing) + .5);
        cairo_show_text(c->cr, buf);
...
add_relative_lines(struct chart *c) {
    ...
    do {
        ...
        cairo_set_source_rgba(c->cr, .75, 0, 0, .95);
        cairo_move_to(c->cr, 1 - extents.x_bearing,
                      floor(mid + y) - (extents.height / 2 + extents.y_bearing) + .5);
        cairo_show_text(c->cr, buf);
        ...
        cairo_move_to(c->cr, 1 - extents.x_bearing,
                      ceil(mid - y) - (extents.height / 2 + extents.y_bearing) + .5);
        cairo_show_text(c->cr, buf);
