...
graph_view_draw(GraphView *self,
    ...
    if (self->selected_report != -1) {
        ...
        {
            double x0, x1, y;
            x0 = self->selected_report;
            y = 0;
            cairo_matrix_transform_point(&m, &x0, &y);
            x0 = floor(x0);
            x1 = self->selected_report + 1;
            y = 0;
            ...
            x0 -= y;
            x1 += y;
            cairo_rectangle(cr, x0, PAD / 2, x1 - x0,
                            self->widget.allocation.height - 2 * PAD + PAD);
...
graph_view_button_press(GtkWidget *w,
    ...
    cairo_matrix_init_translate(&m, PAD, self->widget.allocation.height - PAD);
    cairo_matrix_scale(&m, (self->widget.allocation.width - 2 * PAD) / self->num_reports,
                       -(self->widget.allocation.height - 2 * PAD) / (self->ymax - self->ymin));
