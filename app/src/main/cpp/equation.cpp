...
// at bottom (like lower bound of int) substract fontsize from yp
    ...
    vecteur
    Equation::Equation_subsizes(const gen &arg, const attributs &a, int windowhsize, GIAC_CONTEXT) {
        vecteur v;
    ...
    gen Equation::Equation_compute_symb_size(const gen &g, const attributs &a, int windowhsize,
                                             GIAC_CONTEXT) {
        if (g.type != _SYMB)
        ...
        if (u == at_rootof && arg.type == _VECT && arg._VECTptr->size() == 2 &&
            arg._VECTptr->front().type == _VECT &&
            has_rootof_value(arg._VECTptr->back(), rootof_value, contextptr)) {
            return Equation_compute_symb_size(
                    horner_rootof(*arg._VECTptr->front()._VECTptr, rootof_value, contextptr), a,
                    windowhsize, contextptr);
        }
        if (u == at_multistring) {
        ...
        if (u == at_makesuite) {
            if (arg.type == _VECT)
                return Equation_compute_size(gen(*arg._VECTptr, _SEQ__VECT), a, windowhsize,
                                             contextptr);
            else
                return Equation_compute_size(arg, a, windowhsize, contextptr);
        }
        if (u == at_sqrt)
            return Equation_compute_size(
                    symbolic(at_pow, gen(makevecteur(arg, plus_one_half), _SEQ__VECT)), a,
                    windowhsize, contextptr);
        if (u == at_division) {
...
#else
            tmp.__FRACptr = new ref_fraction(
                    Tfraction<gen>(arg._VECTptr->front(), arg._VECTptr->back()));
#endif
            tmp.type = _FRAC;
            return Equation_compute_size(tmp, a, windowhsize, contextptr);
        }
        if (u == at_prod) {
            ...
            if (rewrite_prod_inv(arg, n, d)) {
                if (n.is_symb_of_sommet(at_neg))
                    return Equation_compute_size(symbolic(at_neg, Tfraction<gen>(-n, d)), a,
                                                 windowhsize, contextptr);
        ...
        if (u == at_inv) {
            if ((is_integer(arg) && is_positive(-arg, contextptr))
                ...
                return Equation_compute_size(symbolic(at_neg, Tfraction<gen>(plus_one, -arg)), a,
                                             windowhsize, contextptr);
            return Equation_compute_size(Tfraction<gen>(plus_one, arg), a, windowhsize, contextptr);
        }
        if (u == at_expr && arg.type == _VECT && arg.subtype == _SEQ__VECT &&
            arg._VECTptr->size() == 2 && arg._VECTptr->back().type == _INT_) {
        ...
        if ((u == at_integrate) || (u == at_sum)) { // Int
            ...
            if ((s == 1) || (s == 2)) { // int f(x) dx and sum f(n) n
                vecteur v(Equation_subsizes(gen(*arg._VECTptr, _SEQ__VECT), a, windowhsize,
                                            contextptr));
        ...
        if (u == at_limit && arg.type == _VECT) { // limit
            ...
            if (s >= 3) {
                ...
                if (s == 4) {
                    ...
                    res.push_back(
                            eqwdata(vf.dx + vx.dx + a.fontsize + 4 + vl.dx + vdir.dx, h, 0, y, a, u,
                                    0));
                } else
                    res.push_back(
                            eqwdata(vf.dx + vx.dx + a.fontsize + 2 + vl.dx, h, 0, y, a, u, 0));
                return gen(res, _SEQ__VECT);
            }
        }
        if ((u == at_of || u == at_at) && arg.type == _VECT && arg._VECTptr->size() == 2) {
            ...
            if (u == at_at && xcas_mode(contextptr) != 0) {
                if (arg2.type == _VECT)
                    arg2 = gen(addvecteur(*arg2._VECTptr, vecteur(arg2._VECTptr->size(), plus_one)),
                               _SEQ__VECT);
                else
                    arg2 = arg2 + plus_one;
            }
        ...
        if (u == at_factorial) {
            ...
            eqwdata vv = Equation_total_size(varg);
            bool paren = need_parenthesis(vv.g) || vv.g == at_prod || vv.g == at_division ||
                         vv.g == at_pow;
            if (paren)
                x += llp;
            Equation_translate(varg, x, 0);
        ...
        if (u == at_program && arg._VECTptr->back().type != _VECT &&
            !arg._VECTptr->back().is_symb_of_sommet(at_local)) {
            gen varg = Equation_compute_size(arg._VECTptr->front(), a, windowhsize, contextptr);
            eqwdata vv = Equation_total_size(varg);
            Equation_vertical_adjust(vv.dy, vv.y, h, y);
            Equation_translate(varg, x, 0);
        ...
        bool binaryop = (u.ptr()->printsommet == &printsommetasoperator) ||
                        equalposcomp(binary_op_tab(), u);
        if (u != at_sto && u.ptr()->printsommet != NULL && !binaryop) {
            gen tmp = string2gen(g.print(contextptr), false);
            return Equation_compute_size(
                    symbolic(at_expr, gen(makevecteur(tmp, xcas_mode(contextptr)), _SEQ__VECT)), a,
                    windowhsize, contextptr);
        }
        vecteur v;
        ...
        if (it == itend || (itend - it == 1)) {
            gen gtmp;
            if (it == itend)
                gtmp = Equation_compute_size(gen(vecteur(0), _SEQ__VECT), a, windowhsize,
                                             contextptr);
            else
                gtmp = *it;
        ...
        if (binaryop) { // op (default with par)
            ...
            for (;;) {
                ...
                if (u == at_plus && it != v.begin() &&
                    (
                            (it->type == _VECT && it->_VECTptr->back().type == _EQW &&
                             it->_VECTptr->back()._EQWptr->g == at_neg)
                            ||
                            (it->type == _EQW &&
                             (is_integer(it->_EQWptr->g) || it->_EQWptr->g.type == _DOUBLE_) &&
                             is_strictly_positive(-it->_EQWptr->g, contextptr))
                    )
                        )
                    x -= ls;
                if (x > windowhsize - vv.dx && x > windowhsize / 2 &&
                    (itend - it) * vv.dx > windowhsize / 2) {
...
// (of subtype _EQW__VECT or _HIST__VECT)
    gen Equation::Equation_compute_size(const gen &g, const attributs &a, int windowhsize,
                                        GIAC_CONTEXT) {
        fl_font(FL_HELVETICA, a.fontsize);
        /**************
   * FL_WIDGET  *
   ...
   *****************/
        if (g.type == _FRAC) {
            if (is_integer(g._FRACptr->num) && is_positive(-g._FRACptr->num, contextptr))
                return Equation_compute_size(
                        symbolic(at_neg, fraction(-g._FRACptr->num, g._FRACptr->den)), a,
                        windowhsize, contextptr);
   ...
   ***************/
        if ((g.type == _VECT) && !g._VECTptr->empty()) {
            if (g.subtype == _SPREAD__VECT)
                return Equation_compute_size(string2gen("spreadsheet", false), a, windowhsize,
                                             contextptr);
            vecteur v;
  ...
  gen res=gen(v,_HIST__VECT); Equation_translate(res,0,y); return res;
     ...
     ***************/
            ...
            if (gmat && g.subtype != _SEQ__VECT && g.subtype != _SET__VECT &&
                g.subtype != _POLY1__VECT && g._VECTptr->front().subtype != _SEQ__VECT) {
                gen mkvect(at_makevector);
                ...
                if (ncols) {
                    ...
                    for (int i = 0; it != itend; ++it, ++i) {
                        gen tmpg = *it;
                        tmpg.subtype = _SEQ__VECT;
                        vecteur tmp(Equation_subsizes(tmpg, a,
                                                      max(windowhsize / ncols - a.fontsize, 230),
                                                      contextptr));
                    ...
                    for (int i = 0; it != itend; ++it, ++i) {
                        ...
                        it->_VECTptr->push_back(
                                eqwdata(col_widths.back(), h - prev_h, 0, -h, a, mkvect, 0));
                        prev_h = h;
                    }
                    all_sizes.push_back(
                            eqwdata(col_widths.back(), row_heights.back(), 0, -row_heights.back(),
                                    a, mkmat, -row_heights.back() / 2));
   ...
   **********************/
    ...
    bool Equation_multistring_selection(const_iterateur &it, const_iterateur &itend,
                                        bool search_active) {
    ...
    void
    Equation::Equation_draw(const eqwdata &e, int x, int y, int rightx, int lowery, Equation *eq,
                            int begin_sel, int end_sel) {
        ...
        if (gg.type == _STRNG) {
            ...
            for (pos = 0; pos < ss; ++pos) {
                char ch = s[pos];
                if (ch == '\n') {
                    check_fl_draw(cs.c_str(), eq->x() + e.x - x, eq->y() + y - e.y + vsize - e.dy,
                                  eq->clip_x, eq->clip_y, eq->clip_w, eq->clip_h, 0, 0);
                    cs = "";
                    vsize += fontsize;
                } else
                    cs += ch;
            ...
            if (selected && css) {
                ...
                fl_color(text_color);
                check_fl_rectf(eq->x() + e.x - x + deltax, eq->y() + y - e.y - e.dy + 1, dx,
                               e.dy + 3, eq->clip_x, eq->clip_y, eq->clip_w, eq->clip_h, 0, 0);
                fl_color(background);
                check_fl_draw(cs.c_str(), eq->x() + e.x - x + deltax,
                              eq->y() + y - e.y + vsize - e.dy, eq->clip_x, eq->clip_y, eq->clip_w,
                              eq->clip_h, 0, 0);
            }
            return;
        }
        if (selected) {
            fl_color(text_color);
            check_fl_rectf(eq->x() + e.x - x, eq->y() + y - e.y - e.dy + 1, e.dx, e.dy + 3,
                           eq->clip_x, eq->clip_y, eq->clip_w, eq->clip_h, 0, 0);
            fl_color(background);
        }
        ...
        // cerr << s.size() << endl;
        check_fl_draw(s.c_str(), eq->x() + e.x - x, eq->y() + y - e.y, eq->clip_x, eq->clip_y,
                      eq->clip_w, eq->clip_h, 0, 0);
        return;
    }

    void
    Equation::Equation_draw(const gen &g, int x, int y, int rightx, int lowery, Equation *equat) {
   ...
   *******************/
        ...
        if (selected) {
            fl_color(text_color);
            check_fl_rectf(eqx + w.x - x, eqy + y - w.y - w.dy + 1, w.dx, w.dy + 3, equat->clip_x,
                           equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
        } else {
            fl_color(background);
        ...
        if (oper == at_multistring) {
            ...
            if (Equation_multistring_selection(it_beg, it_end, false)) {
                ...
                if (it_beg == it_end) {
                    Equation_draw(*it->_EQWptr, x, y, rightx, lowery, equat, equat->begin_sel,
                                  equat->end_sel);
                    ++it;
                } else {
            ...
            for (int i = 0; it != itend; it += skip, ++i) {
                ...
                if (wlevel || !rpn_mode(contextptr))
                    check_fl_draw((print_INT_(wlevel) + ": ").c_str(), eqx - x, eqy + yy,
                                  equat->clip_x, equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
            }
        ...
        if (oper.type == _FUNC) {
            ...
            if (u == at_at) { // draw brackets around 2nd arg
                ...
                fl_font(FL_HELVETICA, fontsize);
                if (x0 < rightx)
                    check_fl_draw("[", eqx + x0 - x - int(fl_width("[")), eqy + y - varg2.baseline,
                                  equat->clip_x, equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                x0 += varg2.dx;
                if (x0 < rightx)
                    check_fl_draw("]", eqx + x0 - x, eqy + y - varg2.baseline, equat->clip_x,
                                  equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                return;
            }
            if (u == at_of) { // do we need to draw some parenthesis?
                gen arg2 = v[1]; // 2nd arg of at_of, i.e. what's inside the parenth.
                if (arg2.type != _VECT || arg2._VECTptr->back().type != _EQW ||
                    arg2._VECTptr->back()._EQWptr->g !=
                    at_makesuite) { // Yes (if not _EQW it's a sequence with parent)
                    eqwdata varg2 = Equation_total_size(arg2);
                    x0 = varg2.x;
                    y0 = varg2.y;
                    y1 = y0 + varg2.dy;
                    ...
                    if (x0 < rightx)
                        check_fl_draw("(", eqx + x0 - x - int(fl_width("(")),
                                      eqy + y - varg2.baseline, equat->clip_x, equat->clip_y,
                                      equat->clip_w, equat->clip_h, 0, 0);
                    x0 += varg2.dx;
                    if (x0 < rightx)
                        check_fl_draw(")", eqx + x0 - x, eqy + y - varg2.baseline, equat->clip_x,
                                      equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
            ...
            if (u == at_makesuite) {
                ...
                if (paren && x0 < rightx)
                    check_fl_draw("(", eqx + x0 - x - int(fl_width("(")) / 2, eqy + y - w.baseline,
                                  equat->clip_x, equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                x0 += w.dx;
                if (paren && x0 < rightx)
                    check_fl_draw(")", eqx + x0 - x - int(fl_width("(")) / 2, eqy + y - w.baseline,
                ...
                for (; it != itend; ++it) {
                    ...
                    if (varg2.x + varg2.dx < rightx)
                        check_fl_draw(",", eqx + varg2.x + varg2.dx - x + 1,
                                      eqy + y - varg2.baseline, equat->clip_x, equat->clip_y,
                                      equat->clip_w, equat->clip_h, 0, 0);
                }
                return;
            ...
            if (u == at_makevector) { // draw [] delimiters for vector/matrices
                if (oper.subtype != _SEQ__VECT && oper.subtype != _PRINT__VECT) {
                    ...
                    if (x0 + 1 < rightx) {
                        check_fl_line(eqx + x0 - x + 1, eqy + y - y0 + 1, eqx + x0 - x + 1,
                                      eqy + y - y1 + 1, equat->clip_x, equat->clip_y, equat->clip_w,
                                      equat->clip_h, 0, 0);
                        check_fl_line(eqx + x0 - x + decal, eqy + y - y0 + 1, eqx + x0 - x + decal,
                                      eqy + y - y1 + 1, equat->clip_x, equat->clip_y, equat->clip_w,
                                      equat->clip_h, 0, 0);
                        check_fl_line(eqx + x0 - x + 1, eqy + y - y0 + 1,
                                      eqx + x0 - x + fontsize / 4, eqy + y - y0 + 1, equat->clip_x,
                                      equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                        check_fl_line(eqx + x0 - x + 1, eqy + y - y1 + 1,
                                      eqx + x0 - x + fontsize / 4, eqy + y - y1 + 1, equat->clip_x,
                                      equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                    }
                    x0 += w.dx;
                    if (x0 - 1 < rightx) {
            ...
            y0 = y - w.baseline;
            if (u == at_pow) {
                if (!need_parenthesis(tmp.g) && tmp.g != at_pow && tmp.g != at_prod &&
                    tmp.g != at_division)
                    return;
                if (tmp.g == at_pow) {
            ...
            if (u == at_sum) {
                if (x0 < rightx) {
                    check_fl_line(eqx + x0, eqy + y0, eqx + x0 + (2 * fontsize) / 3, eqy + y0,
                                  equat->clip_x, equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                    check_fl_line(eqx + x0, eqy + y0 - fontsize, eqx + x0 + (2 * fontsize) / 3,
                                  eqy + y0 - fontsize, equat->clip_x, equat->clip_y, equat->clip_w,
                                  equat->clip_h, 0, 0);
                    check_fl_line(eqx + x0, eqy + y0, eqx + x0 + fontsize / 2,
                                  eqy + y0 - fontsize / 2, equat->clip_x, equat->clip_y,
                                  equat->clip_w, equat->clip_h, 0, 0);
                    check_fl_line(eqx + x0 + fontsize / 2, eqy + y0 - fontsize / 2, eqx + x0,
                                  eqy + y0 - fontsize, equat->clip_x, equat->clip_y, equat->clip_w,
                                  equat->clip_h, 0, 0);
                    if (v.size() > 2) { // draw the =
                        eqwdata ptmp = Equation_total_size(v[1]);
                        if (ptmp.x + ptmp.dx < rightx)
                            check_fl_draw("=", eqx + ptmp.x + ptmp.dx - x - 2,
                                          eqy + y - ptmp.baseline, equat->clip_x, equat->clip_y,
                                          equat->clip_w, equat->clip_h, 0, 0);
                    }
                }
                return;
            ...
            if (u == at_abs) {
                ...
                if (x0 < rightx) {
                    ...
                    check_fl_line(eqx + x0 + 1, eqy + y0 - 1, eqx + x0 + 1, eqy + y0 - h + 3,
                                  equat->clip_x, equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                    check_fl_line(eqx + x0 + w.dx - 1, eqy + y0 - 1, eqx + x0 + w.dx - 1,
                                  eqy + y0 - h + 3, equat->clip_x, equat->clip_y, equat->clip_w,
                                  equat->clip_h, 0, 0);
            ...
            if (u == at_sqrt) {
                ...
                int h = w.dy;
                if (x0 < rightx) {
                    check_fl_line(eqx + x0 + 2, eqy + y0 - h / 2, eqx + x0 + fontsize / 2,
                                  eqy + y0 - 1, equat->clip_x, equat->clip_y, equat->clip_w,
                                  equat->clip_h, 0, 0);
                    check_fl_line(eqx + x0 + fontsize / 2, eqy + y0 - 1, eqx + x0 + fontsize,
                                  eqy + y0 - h + 3, equat->clip_x, equat->clip_y, equat->clip_w,
                                  equat->clip_h, 0, 0);
                    check_fl_line(eqx + x0 + fontsize, eqy + y0 - h + 3, eqx + x0 + w.dx - 1,
                                  eqy + y0 - h + 3, equat->clip_x, equat->clip_y, equat->clip_w,
                                  equat->clip_h, 0, 0);
                    ++y0;
                    check_fl_line(eqx + x0 + 2, eqy + y0 - h / 2, eqx + x0 + fontsize / 2,
            ...
            if (u == at_factorial) {
                check_fl_draw("!", eqx + w.x + w.dx - 4 - x, eqy + y - w.baseline, equat->clip_x,
                              equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
            ...
            if (u == at_integrate) {
                ...
                if (x0 < rightx) {
                    ...
                    fl_arc(eqx + x0, eqy + y0 + fontsize / 3, fontsize / 3, fontsize / 3, 180,
                           360);//adjust for xcas pad
                    check_fl_line(eqx + x0 + fontsize / 3, eqy + y0, eqx + x0 + fontsize / 3,
                                  eqy + y0 - 2 * fontsize + 4, equat->clip_x, equat->clip_y,
                                  equat->clip_w, equat->clip_h, 0, 0);
                    fl_arc(eqx + x0 + fontsize / 3, eqy + y0 - 2 * fontsize + 3, fontsize / 3,
                           fontsize / 3, 0, 180);
                }
                if (v.size() != 2) { // if arg has size > 1 draw the d
                    eqwdata ptmp = Equation_total_size(v[1]);
                    if (ptmp.x < rightx)
                        check_fl_draw(" d", eqx + ptmp.x - x - int(fl_width(" d")),
                                      eqy + y - ptmp.baseline, equat->clip_x, equat->clip_y,
                                      equat->clip_w, equat->clip_h, 0, 0);
                } else {
                    ...
                    if (ptmp.x + ptmp.dx < rightx)
                        check_fl_draw(" dx", eqx + ptmp.x + ptmp.dx - x, eqy + y - ptmp.baseline,
                                      equat->clip_x, equat->clip_y, equat->clip_w, equat->clip_h, 0,
                                      0);
                }
                return;
            }
            if (u == at_division) {
                if (x0 < rightx) {
                    int yy = eqy + y0 - 6;
                    check_fl_line(eqx + x0 + 2, yy, eqx + x0 + w.dx - 2, yy, equat->clip_x,
                                  equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                    ++yy;
                    check_fl_line(eqx + x0 + 2, yy, eqx + x0 + w.dx - 2, yy, equat->clip_x,
                                  equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
            ...
            if (u == at_limit && v.size() >= 4) {
                if (x0 < rightx)
                    check_fl_draw("lim", eqx + w.x - x, eqy + y - w.baseline, equat->clip_x,
                                  equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                ...
                if (v.size() >= 5) {
                    ...
                    if (arg2.type == _EQW) {
                        eqwdata &varg2 = *arg2._EQWptr;
                        if (varg2.x + varg2.dx < rightx)
                            check_fl_draw(",", eqx + varg2.x + varg2.dx - x,
                                          eqy + y - varg2.baseline, equat->clip_x, equat->clip_y,
                                          equat->clip_w, equat->clip_h, 0, 0);
                    }
                }
            ...
            if ((itend - it == 1) && ((u == at_neg)
                ...
                if (w.x < rightx) {
                    ...
                    if (u == at_neg) {
                        int xx = w.x - x;
                        int cx = eqx + xx + 2, cy =
                                eqy + y - w.baseline - fontsize / 3, ch = giacmax(2,
                                                                                  (fontsize + 5) /
                                                                                  6);
    ...
    check_fl_draw(u.ptr()->s,eqx+w.x-x,eqy+y-w.baseline,equat->clip_x,equat->clip_y,equat->clip_w,equat->clip_h,0,0);
            ...
            if (u == at_plus)
                ...
                if (parenthesis && need_parenthesis(tmp.g)) {
                    if (w.x < rightx) {
                        ...
                        fl_font(FL_HELVETICA, pfontsize);
                        check_fl_draw("(", eqx + w.x - x, eqy + y - tmp.baseline, equat->clip_x,
                                      equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                        fl_font(FL_HELVETICA, fontsize);
                    }
                }
            ...
            for (;;) {
                ...
                if (parenthesis && need_parenthesis(tmp.g)) {
                    if (xx < rightx) {
                        int pfontsize = min(max(fontsize, (fontsize + (tmp.baseline - tmp.y)) / 2),
                                            fontsize * 2);
                ...
                ++it;
                if (it == itend) {
                    if (u.ptr()->printsommet == &printsommetasoperator || u == at_sto ||
                        equalposcomp(binary_op_tab(), u))
                        return;
                    else
                        break;
    ...
    fl_line(cx-ch,cy-ch,cx+ch,cy+ch);
                    ...
                    if (u == at_plus && (
                        ...
                        if (xx + 1 < rightx)
                            ...
                            check_fl_draw(opstring.c_str(), eqx + xx + 1, eqy + y - tmp.baseline,
                                          equat->clip_x, equat->clip_y, equat->clip_w,
                                          equat->clip_h, 0, 0);
                    }
                }
                ...
                if (parenthesis && (need_parenthesis(tmp.g))) {
                    if (tmp.x - lpsize < rightx) {
                        ...
                        int deltapary = (2 * (pfontsize - fontsize)) / 3;
                        fl_font(FL_HELVETICA, pfontsize);
                        check_fl_draw("(", eqx + tmp.x - pfontsize * lpsize / fontsize - x,
                                      eqy + y - tmp.baseline + deltapary, equat->clip_x,
                                      equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                        fl_font(FL_HELVETICA, fontsize);
                    }
                }
            ...
            if (w.x < rightx) {
                ...
                s += '(';
                check_fl_draw(s.c_str(), eqx + w.x - x, eqy + y - w.baseline, equat->clip_x,
                              equat->clip_y, equat->clip_w, equat->clip_h, 0, 0);
                fl_font(FL_HELVETICA, fontsize);
            }
